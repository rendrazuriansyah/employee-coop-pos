...
$levels = $petugas->getAllLevels();

$id = $_GET['id'] ?? 0;
$petugasData = $petugas->getById($id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $errors = $petugas->validate($data);
    
    if (empty($errors)) {
        try {
            // Remove current_id from data before updating (it's only for validation)
            unset($data['current_id']);
            $petugas->updateUser($id, $data);
            header('Location: index.php?success=Data petugas berhasil diperbarui');
            exit;
        } catch (Exception $e) {
            $errors[] = 'Gagal memperbarui petugas: ' . $e->getMessage();
        }
    }
} else {
...
<head>
    ...
    <title>Edit Petugas - Koperasi Pegawai RSUD Tarakan</title>
                ...
                <div class="card">
                    <div class="card-header bg-warning text-white">
                        <h5 class="mb-0">
                            ...
                            EDIT PETUGAS: <?= htmlspecialchars($petugasData['nama_user']) ?>
                        </h5>
                    </div>
                                ...
                                <div class="col-md-6">
                                    <div class="mb-3">
                                        ...
                                        <input type="text" class="form-control" id="nama_user" name="nama_user" 
                                               value="<?= htmlspecialchars($_POST['nama_user'] ?? '') ?>" required>
                                    </div>
                                </div>
                                    ...
                                    <div class="mb-3">
                                        ...
                                        <select class="form-select" id="level" name="level" required>
                                            ...
                                            <option value="<?= $lvl['id_level'] ?>" 
                                                    <?= (($_POST['level'] ?? '') == $lvl['id_level']) ? 'selected' : '' ?>>
                                                <?= ucfirst($lvl['level']) ?>
                                            </option>
                                            <?php endforeach; ?>
                                        </select>
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <div class="alert alert-info">
                                        ...
                                        Anda sedang mengedit akun administrator utama. Berhati-hatilah dalam mengubah data ini.
                                        <?php else: ?>
                                        Perubahan akan berlaku setelah petugas login ulang jika mengubah username atau password.
                                        <?php endif; ?>
                                    </div>
                                </div>
                            </div>

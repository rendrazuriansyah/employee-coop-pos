...
if ($search) {
    ...
    $result = $item->paginate($page, $perPage);
    $items = $result['data'];
...
<head>
    ...
    <title>Data Item - Koperasi Pegawai RSUD Tarakan</title>
...
<body>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                ...
                                <tbody>
                                    ...
                                    <?php else: ?>
                                    <?php foreach ($items as $index => $itm): ?>
                                    <?php 
                                    $rowClass = '';
                                    if ($itm['stok'] <= 2) {
                                    ...
                                    <tr class="<?= $rowClass ?>">
                                        ...
                                        <td><?= htmlspecialchars($itm['id_item']) ?></td>
                                        <td><?= htmlspecialchars($itm['nama_item']) ?></td>
                                        <td><?= htmlspecialchars($itm['uom']) ?></td>
                                        <td>Rp <?= number_format($itm['harga_beli'], 0, ',', '.') ?></td>
                                        <td>Rp <?= number_format($itm['harga_jual'], 0, ',', '.') ?></td>
                                        <td>
                                            <span class="badge bg-<?= $itm['stok'] <= 2 ? 'danger' : ($itm['stok'] <= 5 ? 'warning' : 'success') ?>">
                                                <?= $itm['stok'] ?>
                                            </span>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <div class="btn-group btn-group-sm">
                                                <a href="view.php?id=<?= $itm['id_item'] ?>" 
                                                   class="btn btn-info btn-sm" title="View">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <a href="edit.php?id=<?= $itm['id_item'] ?>" 
                                                   class="btn btn-warning btn-sm" title="Edit">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <button type="button" class="btn btn-danger btn-sm" 
                                                        onclick="deleteItem(<?= $itm['id_item'] ?>)" title="Delete">
                        ...
                        <div class="row">
                            <div class="col-md-6">
                                <p class="text-muted">
                                    ...
                                    <?= min($page * $perPage, $totalItems) ?> of 
                                    <?= $totalItems ?> entries
                                </p>
                            </div>
                            <div class="col-md-6">
                        ...
                        foreach ($items as $itm) {
                            if ($itm['stok'] <= 5) $lowStockCount++;
                        }
                        ?>
                        <?php if ($lowStockCount > 0): ?>
                        <div class="alert alert-warning mt-3">
                            <i class="fas fa-exclamation-triangle me-2"></i>
                            <strong>Peringatan:</strong> Ada <?= $lowStockCount ?> item dengan stok menipis (≤5 unit).
                            <a href="#" class="alert-link" onclick="filterLowStock()">Lihat item dengan stok menipis</a>
                        </div>
                        <?php endif; ?>
                    </div>
                </div>
    ...
    <script>
        ...
        function filterLowStock() {
            // Hide all rows except those with low stock
            const rows = document.querySelectorAll('tbody tr');
            rows.forEach(row => {
                if (row.classList.contains('stock-low') || row.classList.contains('stock-critical')) {
                    row.style.display = '';
                } else {
